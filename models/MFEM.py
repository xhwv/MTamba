...
class MSCB(nn.Module):
    def __init__(self, n, outc):
        ...
        self.conv2 = nn.Conv3d(
            in_channels=12,  # 总输出通道数（6+2+6），可根据实际需要调整
            out_channels=outc,
            kernel_size=(1, 1, 1),
            stride=1,
            padding=(0, 0, 0)
...
class SMEM(nn.Module):
    def __init__(self):
        ...
        self.mtc7 = MSCB(n=7,outc=1)
        self.mtc5 = MSCB(n=5,outc=2)
        self.mtc3 = MSCB(n=3,outc=2)
        self.sig = nn.Sigmoid()
    ...
    def forward(self, x, y):
        s_x = x
        s_y = y
        x_max = F.adaptive_max_pool3d(x, output_size=x.shape[2:])  # 保持空间尺寸不变
        x_avg = F.adaptive_avg_pool3d(x, output_size=x.shape[2:])  # 保持空间尺寸不变
        y_max = F.adaptive_max_pool3d(y, output_size=y.shape[2:])  # 保持空间尺寸不变
        y_avg = F.adaptive_avg_pool3d(y, output_size=y.shape[2:])  # 保持空间尺寸不变
